...
EXPECTED_ANSWER = "M5_BEDROCK_SMOKE_OK"
...
def validate_m5_bedrock_smoke_payload(payload: object) -> dict[str, Any]:
    if not isinstance(payload, dict):
        raise ValueError("response must be a JSON object")
    if payload.get("answer") != EXPECTED_ANSWER:
        raise ValueError("answer did not match the M5 smoke contract")
    ...
    if not isinstance(llm_calls, list) or len(llm_calls) != 2:
        raise ValueError("M5 requires exactly two LLM calls")
