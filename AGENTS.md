...
- When the user requests manual testing (e.g., "start docker to test", "i can help with manual testing", "manual testing"), use `.claude/skills/local-docker-test/SKILL.md`. Give the user a short guide that starts with `cd` into the active worktree and reuses targets from the root `Makefile` where available. Add a root target only for a recurring gap; never create a duplicate Makefile. Verify the applicable health endpoint before handoff.
